...
import RNA_UMI.utils.module_StatInfo     as Stat
import RNA_UMI.utils.module_CountInfo    as m_cnt
import RNA_UMI.utils.module_GTFFeature   as m_gtf
import RNA_UMI.utils.module_running_jobs as m_jobs
import RNA_UMI.utils.module_RepeatCount  as m_repcnt
...
#        Cuffnorm_file = "%s/merge.FPKM.gene.xls" % ( self.dir_cufflinks_known_ERCC_mrg )
        Cuffnorm_file = "%s/%s/%s.UMI_TPM.xls" % (self.dir_mergeCout ,self.s_idx, self.s_idx )
...
#                    self.l_RGCs_FPKM[brief_name].append(f[i+1])
    ...
    def __load_umi(self):
        umi_file = "%s/%s/%s.UMI_count.xls" % (self.dir_mergeCout ,self.s_idx, self.s_idx )
        f_umi    = open( umi_file,"r" )
        in_head       = f_umi.readline()
        f_head        = in_head.split()
        l_samp_brief  = [ sam for sam in f_head[1:] ]

        for brief_name in l_samp_brief:
            self.l_ERCC_UMI[brief_name] = []
            self.l_mRNA_UMI[brief_name] = []
        ...
        for line in f_umi:
            ...
            if gene[0:5].upper() == "ERCC-":
                self.l_ERCC_name.append(gene)
                for i,brief_name in enumerate(l_samp_brief):
                    self.l_ERCC_UMI[brief_name].append(f[i+1])

            else:
                self.l_mRNA_name.append(gene)
                for i,brief_name in enumerate(l_samp_brief):
                    self.l_mRNA_UMI[brief_name].append(f[i+1])
...
#        self.__mRNA_refSeq_index()
    ...
    def __mRNA_refSeq_index(self):
        ...
        for i,gene in enumerate(self.l_mRNA_name):   ##without : ERCC- MIR SNORD
