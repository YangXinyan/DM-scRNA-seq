...
class SampStat(m_scpt.Scripts):
    ...
    def Basic_Stat(self):
        ...
        if not self.given_GTF:
            l_info = [
                "Rename", "Brief_name", "Raw_Reads", "Raw_bases", "Clean_Reads", "Clean_bases", "Q20(%)","Q30(%)","GC content(%)",
...
#            NeoPass_info.sam_tot_reads()
        ...
        for samp in self.samInfo_pd_RNA['sample']:
            ...
            HTSeq_SpikeIn = "%s/%s/%s.umi_clean_ERCC.xls"             %\
...
#        self.__load_Count()
        ...
        l_info = [
            "Sample", "Brief_samp", "ERCC_MOLs", "mRNA_MOLs",
            "RefSeq_mRNA_MOLs", "Regression_R",
            "Regression_P", "RefSeq_mRNA_TPM>0"
        ] 
